...
/// full 21-channel frame to atlas-api (which heartbeats it to the bridge —
/// no show required).
@MainActor
@Observable
final class LightsModel {
    ...
    /// Debounced push — dragging a color wheel fires continuously, the server
    ...
    /// Pull the server's held frame so the board matches reality after launch.
    func sync() async {
        do {
