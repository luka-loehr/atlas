...
final class DashboardModel {
    ...
    // network rates in Mbit/s, derived from the API server's cumulative counters
    ...
    /// One JSON text frame every 500 ms, see the API server's wire format.
    ...
    private func apply(_ f: WSFrame) {
        ...
        if let last = lastWSNet {
            // cumulative counters: a smaller value means the server restarted — skip that delta
    ...
    /// Seeds the buffers from the API server's 10-minute ring buffer so charts are
