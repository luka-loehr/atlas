...
from a phone. It is the backend of the [Atlas Admin](../apps/atlas-admin)
and [Atlas Lightshow](../apps/atlas-lightshow) iOS apps, but every route is
plain HTTP/JSON or WebSocket and works just as well from `curl`.
...
| `/api/vpn` | Tailscale backend state + version, exit-node offer, own DNS name, peers (`host`, `os`, `online`, `active`, `rx`, `tx`, `last_seen`), tunnel-usage accumulators (`since`, `tunnel_s`, `bytes`), AdGuard stats |
| `/api/activity` | `{"today":"YYYY-MM-DD","days":[…]}` — 154 days × `{"d","min","boots","commits"}` |
...
- **Token (recommended):** set `ATLAS_API_TOKEN`. Every request must carry
  `Authorization: Bearer <token>`; WebSocket clients may pass `?token=…`
  instead. Comparison is constant-time.
- **No token (default):** the server fails closed — read-only `GET` routes are
  served, but the state-changing surface (the PTY terminal and everything
  non-`GET`) is refused with `403`.
- **`ATLAS_API_OPEN=1`:** explicit opt-in to serve state-changing routes
...
(`ATLAS_API_BIND`) if that matters.
...
`atlas api` (build + install on the server over SSH), `atlas api
logs|status|stop|restart`.

## Configuration

All optional, via environment variables — the unit loads
`/etc/atlas-api.env` if present:

| Variable | Default | Purpose |
|---|---|---|
| `ATLAS_API_TOKEN` | *(unset)* | bearer token; strongly recommended |
| `ATLAS_API_OPEN` | *(unset)* | `1` allows state-changing routes without a token |
...
- **`/api/vpn`:** needs Tailscale installed and logged in; AdGuard stats need
  ...
  `$HOME/.local/share/atlas-api/vpn.json` across restarts.
- **Lightshow routes:** need the [lightshows](../lightshows) subsystem
  ...
  bridge `bridge/credentials.json`, the fog hardware). The server talks to the
