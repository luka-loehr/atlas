//! atlas-api — metrics + a real terminal + docker/lightshow/fog control for
//! the Atlas Admin and Atlas Lightshow iOS apps.
//!
//!   GET  /health                 -> {"ok":true}
//!   GET  /api/metrics  (alias /) -> machine snapshot
...
//!   GET  /api/shows              -> bridge status + lightshows on disk
//!   POST /api/shows/create       -> makeshow.py in the background (body: [ai ]<url>)
//!   GET  /api/shows/create/status-> creation progress (phase/percent/log tail)
//!   GET  /api/shows/create/thumb -> cover of the show being created
//!   GET  /api/shows/thumb/<name> -> a show's cover
//!   GET  /api/shows/audio/<name> -> a show's audio file
//!   POST /api/shows/start        -> start the bridge + play a show  (body: name)
//!   POST /api/shows/stop         -> stop the player, keep the bridge
//!   POST /api/bridge/stop        -> stop the player AND the bridge
//!   POST /api/fog                -> fog burst                        (body: ms)
//!   POST /api/fog/stop           -> end the burst early
...
//!   GET  /api/calibrate          -> saved audio latency
//!   POST /api/calibrate/save     -> store audio latency              (body: ms)
...
//!
//! Auth: if ATLAS_API_TOKEN is set, every request needs
...
//! and everything non-GET — are refused unless ATLAS_API_OPEN=1 explicitly
...
fn main() {
    // ATLAS_API_PORT: listen port (default 8787)
    let port: u16 = std::env::var("ATLAS_API_PORT")
        .ok()
        .and_then(|p| p.parse().ok())
        .unwrap_or(8787);
    // ATLAS_API_TOKEN: bearer token; when set, required on every request
    let token = std::env::var("ATLAS_API_TOKEN").ok().filter(|t| !t.is_empty());
    // ATLAS_API_OPEN=1: explicitly allow state-changing routes without a
    // token, for deployments where the tailnet/firewall is the only boundary
    let open = std::env::var("ATLAS_API_OPEN").is_ok_and(|v| v == "1");
    // ATLAS_API_BIND: full listen address (e.g. "127.0.0.1:8787" or a
    // tailscale IP), overrides ATLAS_API_PORT. Rely on the tailnet and a
    // firewall for reachability — never port-forward this service.
    let bind = std::env::var("ATLAS_API_BIND")
        .ok()
        .filter(|b| !b.is_empty())
        .unwrap_or_else(|| format!("0.0.0.0:{port}"));

    let listener = TcpListener::bind(bind.as_str()).unwrap_or_else(|e| {
        eprintln!("atlas-api: cannot bind {bind}: {e}");
        std::process::exit(1);
    });
    vpn::start_sampler();
    stream::start_sampler(); // 10-min metrics history from boot
    println!(
        "atlas-api {} on {bind} (auth: {})",
        env!("CARGO_PKG_VERSION"),
        if token.is_some() {
            "token"
        } else if open {
            "OPEN — no token, state changes allowed (ATLAS_API_OPEN=1)"
        } else {
...
fn handle(mut stream: TcpStream, token: Option<&str>, open: bool) {
    ...
    // Without a configured token these fail closed; ATLAS_API_OPEN=1 is the
    ...
    if mutating && !can_act {
        return respond(
            &mut stream,
            403,
            r#"{"error":"this route changes state: set ATLAS_API_TOKEN (or ATLAS_API_OPEN=1 to explicitly trust the network instead)"}"#,
        );
    }
    ...
    match (req.method.as_str(), req.path.as_str()) {
        ...
        ("GET", "/ws/metrics") if req.ws_key.is_some() => {
            clear_timeouts(&stream); // long-lived WebSocket
            stream::handle_ws(stream, &req.ws_key.unwrap());
        }
